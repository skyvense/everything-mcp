...
- 🔍 **文件搜索**: 支持通过文件名、路径、扩展名等多种方式搜索文件
- 📁 **按扩展名搜索**: 快速查找特定类型的文件（如 .txt, .pdf, .jpg 等）
- 📂 **路径搜索**: 在指定目录中搜索文件
...
**参数:**
- `query` (string, 必需): 搜索关键词，支持文件名、路径、扩展名等
...
**参数:**
- `extension` (string, 必需): 文件扩展名，例如: txt, pdf, jpg (不需要点号)
- `max_results` (integer, 可选): 最大返回结果数量，默认 100
...
**参数:**
- `path` (string, 必需): 搜索路径，例如: C:\Users\Documents
- `query` (string, 可选): 搜索关键词（可选）
- `max_results` (integer, 可选): 最大返回结果数量，默认 100
